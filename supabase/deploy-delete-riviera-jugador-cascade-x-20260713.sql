-- ═══════════════════════════════════════════════════════════════════════════════
-- APLICAR AHORA — Opción X: origen borra → limpia clones en TODOS los clubes
-- Fecha: 2026-07-13
--
-- Incluye:
--   • Cascade a clones (OPA + profile_link mismo key)
--   • Fix FK identity.canonical ANTES del DELETE (evita 409)
--   • Blocklist INSERT interno (evita "Sin permiso" corto de register_*)
--   • Solo auth.uid() = p_organizador_id (club de registro)
--   • NO toca tournaments/pairs/matches/gameses/players
--
-- Ejecutar TODO el archivo en SQL Editor (prod). Luego probar borrar 1 cedido
-- desde el club origen y verificar que desaparece en receptores.
-- ═══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION
      'Sin sesión autenticada. Reintenta tras iniciar sesión.';
  END IF;
