-- =============================================================================
-- HOTFIX P0 (SQL Editor) — espejo de migrations/0021_p0_career_rpc_grants_auth_guards.sql
-- NO deploy automático. Validar verify BEFORE/AFTER.
--
-- P0 quirúrgico (2026-08-08) — GRANTs mínimos + guards de autorización
--
-- Alcance exclusivo:
...
--
-- NO cambia: cálculo de puntos, ROMC emitter rules, delete linked,
-- riviera_list_career_participaciones_public, firmas, payloads ni dual-write.
-- =============================================================================

-- ── 1) ensure_official_profile_link_for_participacion: gate NULL-org + anon ──
...
BEGIN
  ...
  -- P0: anon nunca escribe links (aunque tenga EXECUTE residual).
  IF v_role = 'anon' THEN
    RETURN jsonb_build_object(
  ...
  -- P0: authenticated (no master) siempre autoriza el INSERT.
  -- p_organizador_id NULL ya NO salta el check: exige ownership del jugador.
  IF v_role = 'authenticated' AND NOT public.is_master_admin() THEN
...
-- ── 2) try_write_riviera_official_ledger: guard JWT + ownership (cuerpo RANK-001) ──
...
BEGIN
  ...
  -- P0: anon no escribe ledger. authenticated debe poseer el perfil local
  -- (o ser master). service_role / SQL editor (role vacío) siguen para repairs.
  IF v_role = 'anon' THEN
  ...
  IF v_role = 'authenticated' THEN
    ...
    IF auth.uid() IS NULL
      ...
      RETURN jsonb_build_object('status', 'skipped', 'reason', 'permission_denied');
    END IF;
  END IF;
...
-- ── 3) REVOKE/GRANT mínimos (defensa en profundidad; idempotente) ───────────
REVOKE ALL ON FUNCTION public._riviera_orphan_profile_audit() FROM PUBLIC;
REVOKE ALL ON FUNCTION public._riviera_orphan_profile_audit() FROM anon, authenticated;

REVOKE ALL ON FUNCTION public._riviera_profile_link_resolution(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._riviera_profile_link_resolution(uuid) FROM anon, authenticated;

REVOKE ALL ON FUNCTION public.try_write_riviera_official_ledger(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.try_write_riviera_official_ledger(uuid) FROM anon;
...
REVOKE ALL ON FUNCTION public.ensure_official_profile_link_for_participacion(uuid, uuid) FROM anon;
...
-- ── 4) Verificación de GRANTs a anon ────────────────────────────────────────
DO $$
DECLARE
  v_anon int;
BEGIN
  ...
  IF v_anon <> 0 THEN
    RAISE EXCEPTION 'P0 incompleto: % función(es) P0 siguen con EXECUTE para anon', v_anon;
  END IF;

  IF to_regprocedure('public.try_write_riviera_official_ledger(uuid)') IS NULL THEN
    RAISE EXCEPTION 'P0 incompleto: try_write_riviera_official_ledger ausente';
  END IF;
  IF to_regprocedure('public.ensure_official_profile_link_for_participacion(uuid,uuid)') IS NULL THEN
    RAISE EXCEPTION 'P0 incompleto: ensure_official_profile_link_for_participacion ausente';
  END IF;
  IF to_regprocedure('public.refresh_jugador_stats(uuid)') IS NULL THEN
    RAISE EXCEPTION 'P0 incompleto: refresh_jugador_stats ausente';
  END IF;
END $$;
