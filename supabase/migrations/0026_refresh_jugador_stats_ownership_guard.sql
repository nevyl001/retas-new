-- 0026 — P0: ownership guard en refresh_jugador_stats (SECURITY DEFINER)
--
-- Hallazgo auditoría: cualquier `authenticated` podía refrescar stats de
-- cualquier jugador_id. Alineado con guards de 0021
-- (try_write_riviera_official_ledger / registrar_participacion_jugador):
--   - anon → denegado
--   - authenticated dueño del perfil (rj.organizador_id = auth.uid()) → ok
--   - master admin → ok
--   - service_role / role vacío (repairs SQL) → ok
--
-- NO cambia la semántica algebraica de puntos (SUM sin GREATEST clamp).
...
BEGIN
  ...
  -- P0 ownership (mismo patrón 0021)
  IF v_role = 'anon' THEN
