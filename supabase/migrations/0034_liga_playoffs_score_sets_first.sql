-- ══════════════════════════════════════════════════════════════════════════════
-- parejas_fijas_playoffs: PRIMERO mandan los SETS (no games totales).
--
-- - Cada set: gana quien tiene más games (no hace falta llegar a 6; sin empate).
-- - 2-0 en sets → ganador deportivo; gameDiff > 2 → holgada; 1–2 → cerrada.
-- - 1-1 en sets → STB obligatorio (games NO evitan el STB).
-- - WO 6-0 prioridad absoluta.
--
-- CREATE OR REPLACE de update_liga_partido_score_parejas_fijas_playoffs.
-- NO toca update_liga_partido_score_parejas_fijas (legacy).
-- NO modifica 0030/0031/0032/0033 (ya aplicadas); esta es la corrección sets-first.
-- ══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_wo THEN
    ...
    v_score2 := p_score2;
  ELSE
    -- Sets obligatorios; totales se recalculan (no confiar en p_score*).
    ...
    -- Cada set debe tener ganador (sin empate). No hace falta llegar a 6.
    IF v_s1p1 = v_s1p2 OR v_s2p1 = v_s2p2 THEN
      RETURN jsonb_build_object('ok', false, 'error', 'set_tie_not_allowed');
    END IF;
    ...
    -- STB depende EXCLUSIVAMENTE de empate 1-1 en sets (NO de games totales).
    v_needs_stb := (v_sets_won_p1 = 1 AND v_sets_won_p2 = 1);
...
COMMENT ON FUNCTION public.update_liga_partido_score_parejas_fijas_playoffs(uuid, integer, integer, jsonb, boolean) IS
  'parejas_fijas_playoffs: sets-first. 2-0 sets → ganador; gameDiff clasifica holgada/cerrada. 1-1 sets exige STB. WO 6-0. No confía en totales del cliente.';
