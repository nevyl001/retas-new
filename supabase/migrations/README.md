...
| `0011_align_reta_dynamic_blocks_stage.sql` | Hotfix integración: renombra `generation_reason` → `stage` (y valores `initial`/`dynamic` → `initial_round_robin`/`dynamic_round`) cuando un borrador previo dejó la tabla desalineada del RPC `begin_dynamic_team_block`; sin esto el INSERT del RPC devolvía `42703` → HTTP 400 |
| `0012_align_tournaments_dynamic_team_columns.sql` | Hotfix integración: agrega `tournaments.format` (text) y `tournaments.team_config` (jsonb), backfill desde `tournament_public_config`, completa bloques `generating` que ya tienen partidos; sin esto PATCH/commit de alineación dinámica devolvía `42703` → HTTP 400 y la UI no listaba partidos |
| `0013_retry_orphan_dynamic_blocks.sql` | Hotfix: `retry_dynamic_team_block` también libera bloques `completed` sin partidos (huérfanos tras Resetear), para poder volver a iniciar |
