-- Patch: /jugar distingue Round Robin / Remontada / Reta por Equipos
-- (mode_type `reta` es compartido). Expone tournament_format + championship_enabled.
-- Idempotente. Aplicar en SQL Editor tras revisar.
-- Base: patch-convocatoria-public-entry-fotos.sql
...
BEGIN
  ...
  ELSE
    -- reta (incluye round_robin / remontada / equipos) + americano
    SELECT
      nullif(trim(t.name), ''),
      t.description,
      ...
      t.programado_en,
      t.programado_hasta,
      nullif(trim(coalesce(t.format::text, '')), '')
    INTO
      v_title,
      v_desc,
      v_started,
    ...
    -- Preferir format / championship de tournament_public_config cuando exista
    SELECT
      coalesce(
      ...
      coalesce(
        (tpc.championship_config->>'championshipEnabled')::boolean,
        false
      )
    INTO
      v_tournament_format,
