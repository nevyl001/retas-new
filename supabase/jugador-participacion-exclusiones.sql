...
-- Ejecutar después de riviera-official-multi-club-romc1.sql.
...
ALTER TABLE public.jugador_participacion_exclusiones ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS jpe_select_auth ON public.jugador_participacion_exclusiones;
CREATE POLICY jpe_select_auth ON public.jugador_participacion_exclusiones
  FOR SELECT TO authenticated
  USING (deleted_by_organizador_id = auth.uid());

GRANT SELECT ON public.jugador_participacion_exclusiones TO authenticated;

-- Solo lectura: _resolve_official_player_key (no crear identidad ROMC en el check).
...
COMMENT ON FUNCTION public.is_jugador_participacion_excluded(uuid, text, uuid) IS
